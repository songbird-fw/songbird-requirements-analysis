...
| **RGFN21** | G | N | S | Reliability | The system must support automatic failover of upstream DNS servers: if the primary server responds with timeout for 3 consecutive attempts, the system must automatically switch to the secondary server. | - |
| **RGFN21.1** | G | N | S | Reliability | DNS failover must occur within 5 seconds from recognizing the primary server timeout, without interrupting existing DNS sessions. | - |
| **RGNM22** | G | N | M | Security / Integrity | All configuration actions performed via Web UI must be recorded in an immutable audit log, including: ISO 8601 timestamp, user, action performed, before value and after value. | - |
| **RGNM22.1** | G | N | M | Security | The configuration audit log must be encrypted on write (AES-256) and not directly modifiable, even by the Admin user, ensuring trace integrity. | - |
| **RGFN23** | G | N | C | Maintainability | The system must support eBPF module updates without restarting the main firewall process (hot-reload), keeping traffic active during the update. | - |
| **RGFN23.1** | G | N | C | Maintainability | eBPF module hot-reload must complete the update within 2 seconds without packet loss (zero packet loss). | - |
| **RGFN24** | G | N | M | Compliance | The system must comply with GDPR standards for log data management, allowing configuration of an anonymization (masking) policy for source IP addresses in traffic logs. | - |
| **RGFN24.1** | G | N | M | Compliance | IP anonymization in logs must be selectively applicable per interface and must be callable/discallable via YAML with immediate validation. | - |
| **RUFC25** | U | F | C | Usability | The Web UI must offer a "Dark Mode" to reduce visual fatigue during night monitoring, with a toggle accessible on every page. | - |
| **RGFM26** | G | F | S | Security | The system must automatically block traffic from malicious IP lists (e.g., integration with Abuse.ch or similar) configurable via YAML, with automatic updates every 6 hours. | - |
| **RGFM26.1** | G | F | S | Security | Malicious IP list updates must occur without traffic interruption (background update), applying new rules in eBPF within 1 second from download completion. | - |
